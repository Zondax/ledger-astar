...
import { txBalances_transfer, txUtility_batch } from './zemu_blobs'
...
describe('Standard', function () {
  ...
  test.concurrent.each(models)('sign utility basic', async function (m) {
    ...
    try {
      ...
      // Wait until we are not in the main menu
      await sim.waitUntilScreenIsNot(sim.getMainMenuSnapshot())
      await sim.compareSnapshotsAndApprove('.', `${m.prefix.toLowerCase()}-utility_batch_basic`)
  ...
  test.concurrent.each(models)('sign utility expert', async function (m) {
    ...
    try {
      ...
      // Wait until we are not in the main menu
      await sim.waitUntilScreenIsNot(sim.getMainMenuSnapshot())
      await sim.compareSnapshotsAndApprove('.', `${m.prefix.toLowerCase()}-utility_batch_expert`)
